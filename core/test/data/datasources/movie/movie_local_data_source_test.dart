...
void main() {
  ...
  group('remove watchlist', () {
    test('should return success message when remove from database is success',
        () async {
      // arrange
      when(mockDatabaseHelper.removeWatchlist(1)).thenAnswer((_) async => 1);
      // act
    ...
    test('should throw DatabaseException when remove from database is failed',
        () async {
      // arrange
      when(mockDatabaseHelper.removeWatchlist(1)).thenThrow(Exception());
      // act
      final call = dataSource.removeWatchlist(1);
      // assert
      expect(() => call, throwsA(isA<DatabaseException>()));
    });
  });
